...
class SeqAETSQmlp(nn.Module):
    def __init__(
        ...
        self.enc = MLPEncoder(
            dim_data=dim_data, dim_latent=dim_a * dim_m, act=activation_fxn
        ).to(self.device)
        self.dec = MLPDecoder(
            dim_latent=dim_a * dim_m, dim_data=dim_data, act=activation_fxn
        ).to(self.device)
    ...
    def encode(self, xs):
        ...
        if hasattr(self, "change_of_basis"):
            H = H @ repeat(
                self.change_of_basis, "a1 a2 -> n t a1 a2", n=H.shape[0], t=H.shape[1]
            )
        return H
    ...
    def decode(self, H):
        if hasattr(self, "change_of_basis"):
            H = H @ repeat(
                torch.linalg.inv(self.change_of_basis),
                "a1 a2 -> n t a1 a2",
                n=H.shape[0],
                t=H.shape[1],
            )
    ...
    def dynamics_fn(self, xs, return_loss=False, fix_indices=None):
        ...
        return self.dynamics_model(
            H, return_loss=return_loss, fix_indices=fix_indices, eta=self.eta
        )
    ...
    def loss(
        ...
        xs_pred = self(
            xs_cond,
            gelement=self.g_true,
            return_reg_loss=return_reg_loss,
        ...
        if reconst:
            ...
            xs_target_recon = xs[
                :, T_cond:, :
            ]  # 0:T_cond includes the reconstruction, while T_cond: is the prediction
            xs_target_pred = xs[:, :T_cond, :]
        else:
            xs_target = (
                xs[:, T_cond:] if self.predictive else xs[:, 1:]
            )  # prediction only
        ndim = xs_target.ndim
        loss = torch.mean(
        ...
        if reconst:
            loss_reconst = torch.mean(
                torch.sum(
                    (xs_target_recon - xs_pred[:, T_cond:, :]) ** 2,
                    axis=tuple(range(2, ndim)),
                )
            )
            loss_pred = torch.mean(
                torch.sum(
                    (xs_target_pred - xs_pred[:, :T_cond, :]) ** 2,
                    axis=tuple(range(2, ndim)),
                )
            )
        else:
    ...
    def __call__(
        ...
        if predictive:
            ...
            H_preds = (
                [H] if reconst else []
            )  # if reconst, H_preds[:,0:1,:,:] contains the encoder output for xs_cond[:,0:2,:]
        ...
        for _ in array:
            ...
            H_preds.append(
                H_last
            )  # if reconst, H_preds[:,0:2,:,:] contains encoder outputs, while H_preds[:,2:,:,:] is the recursive outputs of predictor fn
    ...
    def testing(
        ...
        loss = torch.mean(
            torch.sum(
                (xs_target - x_preds) ** 2, axis=tuple(range(2, ndim))
            )  # mean squared error all batch data
        )
