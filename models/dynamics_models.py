...
def loss_orth(M_star):
    ...
    return torch.mean(
        torch.sum((I - M_star @ M_star.transpose(-2, -1)) ** 2, axis=(-2, -1))
    )
...
class LinearTensorDynamics(nn.Module):
    class DynFn(nn.Module):
        ...
        def loss(self, H0, H1):
            ...
            loss = torch.mean(
                torch.sum(
                    dif**2, axis=tuple(range(2, H0.ndim))
        ...
        def __call__(self, H0, H1, indices="None"):
            ...
            if self.fix_indices is not None:
                ...
                active_indices = np.array(
                    list(set(np.arange(dim_a)) - set(self.fix_indices))
                )
    ...
    def __call__(
        ...
        if return_loss:
            ...
            losses = {
                "loss_bd": loss_bd(dyn_fn.M, self.alignment),
